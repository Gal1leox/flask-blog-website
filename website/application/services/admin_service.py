...
class AdminService:
    ...
    def get_records(self, table: str) -> Tuple[List[Any], List[str]]:
        records = self._repo.query_for(table).order_by().all() if table else []
    ...
    def delete_one(self, table: str, record_id: int) -> Tuple[bool, str, int]:
        ...
        model_query = self._repo.query_for(table)
        if not model_query:
            return False, f"Table '{table}' not found.", 404

        record = model_query.get(record_id)
        if not record:
    ...
    def delete_all(self, table: str) -> Tuple[bool, str, int, int]:
        ...
        if table == "users":
            model_query = model_query.filter(
                self._repo.all_tables()[table].role != UserRole.ADMIN
            )

        count = self._repo.bulk_delete(model_query)
        return True, f"Deleted {count} records.", 200, count

    def download_database(self) -> str:
        db_path = os.path.join(current_app.root_path, "..", "instance", Config.DB_NAME)
        return os.path.abspath(db_path)

    def restore_database(self, file: FileStorage) -> Tuple[bool, str]:
        if not file or not file.filename.endswith(".db"):
            return False, "Invalid file. Must be .db"
        target = os.path.join(current_app.root_path, "..", "instance", Config.DB_NAME)
        file.save(target)
